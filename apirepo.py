...
mongodb_client = PyMongo(app, uri="mongodb://%s:%s@%s:%s/api_repo?authSource=admin" % (mongodb_user,mongodb_password,mongodb_host,mongodb_port))
...
#If this endpoint is triggered it calls the APIClarity database, pull all reconstructed APISpecs and updates the internal API Repository with a the new version.
@app.route("/update")
def update_api_repository():
    response = requests.get('http://%s:%s/api_inventory' % (apiclarity_host,apiclarity_port)) #request against APIClarity postgresql db via PostgREST
    ...
    for spec in apiclarity_specs:
        if has_open_api_spec_defined(spec):
            ...
            print("API Spec was updated for %s:%s. New version: %s" % (updated_spec_entry.name, updated_spec_entry.port, updated_spec_entry.version))
            update_counter+=1
...
def save_api_spec_entry_to_db(api_spec_entry):
    ...
    api_spec_entry.__dict__["createdAt"] = created_at_date #add timestamp to entry
...
def get_new_version_of_api_spec(spec):
    ...
    new_version = '%d.%d.%d' % (version.get_major_version(), version.get_minor_version(), (version.get_build_version()+1))
    return new_version