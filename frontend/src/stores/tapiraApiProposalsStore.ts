...
class TapiraApiProposalsStore {
    ...
    async getServicesWithProposedSpecs() {
        const result =  await tapiraApiProposalsService.getServicesWithProposedSpecs();
    }

    @action
    async getProposedSpecsForService(serviceName: string) {
        const result =  await tapiraApiProposalsService.getProposedSpecsForService(serviceName);
    }

    @action
    async getConflictsForService(serviceName: string) {
        const result =  await tapiraApiProposalsService.getConflictsForService(serviceName);
    }
}
